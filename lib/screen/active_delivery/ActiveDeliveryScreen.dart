          ...
          children: [
            ...
            Expanded(
              ...
              onTap: () async {
                ...
                List<double>? coordinates = model.orderid?.deliveryaddress?.location?.coordinates;
                ...
                if (coordinates == null || coordinates.isEmpty) {
                  ...
                  print('Fetched device coordinates: $coordinates');
                }
              ...
              child: Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                if(model.orderid!.userId!=null)
                                  Text("${model.orderid!.userId!.name??""}",
                                      style: TextStyle(
                                          fontSize: 16,
                                          fontWeight: FontWeight.w600)),
                                if(model.orderid!=null)
                                ...
                                Row(
                                  children:  [
                                    ...
                                    if(model.orderid!=null)
                                    if(model.orderid!.userId!=null)
                                      Text("+91 ${model.orderid!.userId!.mobileno}"),
                                    Spacer(),
                          ...
                          if(model.orderid!.items!=null)
                            if(model.orderid!.items!=[])
                              Column(
                                children: model.orderid!.items!.map((toElement){
                                  return  _orderItem(toElement);
                                }).toList(),
                              ),

                          const SizedBox(height: 10),
                          if(model.orderid!=null)
                          Text("Sub Total : ₹${model.orderid!.subtotal??"0"}"),
                          // if(model.orderid!=null)
                          ...
                          Row(
                            ...
                            MainAxisAlignment.spaceBetween,
                            children:  [
                              Text("Payment Method - ${model.orderid!.paymentmethod??""}",
                                  style:
                                  TextStyle(fontWeight: FontWeight.w500)),
                            ],
                          ),
                          ...
                          Row(
                            ...
                            MainAxisAlignment.spaceBetween,
                            children:  [
                              Text("Order Status - ${model.orderid!.status??""}",
                                  style:
                                  TextStyle(fontWeight: FontWeight.w500)),
                            ],
                          ),
                          ...
                          if(model.orderid!.status=="OFD")
                           CustomWidget.stringTypeDropDown(controller.currentStatus, controller.taskStatusList, (value){
                             controller.currentStatus=value!;
                             controller.update();
                           }, controller.statusNode,hideLabel: true),
                           SizedBox(height: 10,),
                          if(model.orderid!.status=="OFD")
                          if(controller.currentStatus==controller.taskStatusList[0])
                            Row(
                              children: [
                                SizedBox(width: 4,),
                                Text("Delivered To",style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500),),
                              ],
                            ),
                          if(model.orderid!.status=="OFD")
                          if(controller.currentStatus==controller.taskStatusList[1])
                            Row(
                              children: [
                                SizedBox(width: 4,),
                                Text("Cancel Reason",style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500),),
                              ],
                            ),
                          if(model.orderid!.status=="OFD")
                           if(controller.currentStatus==controller.taskStatusList[0])
                             CustomWidget.stringTypeDropDown(controller.deliveredTo, controller.deliveredToList, (value){
                               controller.deliveredTo=value!;
                               controller.update();
                             }, controller.deliverNode,hideLabel: true),
                          if(model.orderid!.status=="OFD")
                          if(controller.currentStatus==controller.taskStatusList[1])
                            CustomWidget.stringTypeDropDown(controller.currentReason, controller.cancelReasonList, (value){
                          ...
                          Row(
                            children: [
                              ...
                              Spacer(),
                              Transform.scale(
                                scale: 0.8, // reduce size (0.5 = half, 1.0 = normal, >1 = bigger)
                                child: Switch(
                                  value: controller.isAmountCollected,
                                  activeColor: Colors.green,
                                  onChanged: (value) {
                                    controller.toggleAmountCollected(value);
                                  },
                                ),
                              )
                            ],
                          ...
                          if(model.orderid!.status=="OFD")
                          if(controller.currentStatus==controller.taskStatusList[0]  && controller.isAmountCollected)
                            CustomWidget.textInputFiled(
                          ...
                          Row(
                            children: [
                              Expanded(
                                child:CustomWidget.elevatedCustomButton(context, "Submit",(){
                                  if(controller.currentStatus==controller.taskStatusList[1]) {
                                    controller.cancelOrder(context, model.orderid!.sId ?? "", model.orderid!.godownInchargeId ?? "");
                                  }else{
                                    controller.deliveredOrder(context, model.orderid!.sId ?? "", model.orderid!.godownInchargeId ?? "");
                                  }
                                },borderRadius:25,)
                              ),
